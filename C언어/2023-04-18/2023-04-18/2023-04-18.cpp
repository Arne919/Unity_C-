...
//현재 책장 현황
void Info(Book* bookshelf)//책 유무확인(있을시에 제목 표기)
{

	for (int i = 0; i < BOOK_MAX_COUNT; i++)//책장 반복
	{
		if (bookshelf[i].title[0] == '\0')//제목이 없으면 책이 없음
		{
			printf("%d. 비어있음\n", i + 1);
		}
		else
		{
			printf("%d. %s", i + 1, bookshelf[i].title);

			if (bookshelf[i].rent == true)
			{
				printf("(대여)");
			}
			printf("\n");
...
void Search(Book* bookshelf)
{
	...
	char book_title[BOOK_TITLE_LENGTH]; //검색할 책

	printf("책을 검색하세요: ");
	scanf_s("%c", book_title, BOOK_TITLE_LENGTH);

	bool compare = false;
	
	for (int bookindex = 0; bookindex < BOOK_MAX_COUNT; bookindex++)
	{
		for (int i = 0; i < BOOK_TITLE_LENGTH; i++)//책의 제목의 인덱스
		{
			if (book_title[i] != bookshelf[bookindex].title[i])
			{
				break;
				//서로 다르다
			}
			else
			{
				if (book_title[i] == '\0')
				{
					printf("%s\n", bookshelf[bookindex].title);
					printf("%s\n", bookshelf[bookindex].maker);
					printf("%d\n", bookshelf[bookindex].price);
					return;
				}
			}
			//다를때
...
int Rent(Book* bookshelf)
{
	...
	{
		if (!bookshelf[shelf_num - 1].rent)
		{
			bookshelf[shelf_num - 1].rent = true;
